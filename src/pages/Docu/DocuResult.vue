...
const analysisResult = computed(() => analysisStore.contents || {})

// console.log(contractStore.contents, registerStore.contents.data, analysisStore.contents)
...
const contractResult = {
  ...
  '계약 위험 분석 결과': {
    '보증금 반환 조건 명시 여부': {
      답변: '아니오',
      경고: '계약서에 보증금 반환 시기가 명시되어 있지 않아요. 반환 시점을 명확히 하거나, 확정일자를 등록하는 등의 조치가 필요해요.',
      '근거 조항': '제5조',
    },
    '과도한 위약금 조항 존재 여부': {
      답변: '아니오',
    ...
    '일방적인 계약 해지 조건 존재 여부': {
      답변: '예',
      경고: '임대인이 임차인의 위반 사항에 대해 즉시 계약을 해지할 수 있는 권한을 가지고 있어요. 이는 임차인에게 불리할 수 있으니, 이 부분에 대해 재협의가 필요해요.',
      '근거 조항': '제4조',
    },
  ...
  '조항별 요약 및 설명': {
    '보증금 반환 조건':
      '계약 종료 후 임차인에게 보증금을 반환하며, 연체 임대료 또는 손해배상금이 있을 때는 이들을 제하고 그 잔액을 반환한다는 내용이 있어요. 하지만 구체적인 반환 시점이 명시되어 있지 않아요.',
    '일방적인 계약 해지 조건':
      '임차인이 계약을 위반하였을 때, 임대인이 즉시 계약을 해지할 수 있다는 내용이 있어요. 이는 임차인에게 불리한 조항이므로 재협의가 필요해요.',
  },
}
...
const dangerLists = computed(() => {
  const result = []
  const dangerData = contractResult['계약 위험 분석 결과']
...
const preventLists = computed(() => {
  const result = []
  const dangerData = contractResult['사기 유형 및 대처 방안']
  for (const key in dangerData) {
    const item = dangerData[key]
    result.push({
      제목: key,
...
const totalScore = computed(() => {
  const contractScore = parseInt(contractResult['전체 위험도 점수'])
